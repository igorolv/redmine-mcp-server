...
public record ProjectSummary(
        @Schema(description = "Project identifier the summary was computed for.", requiredMode = Schema.RequiredMode.REQUIRED)
        String projectId,
        @Schema(description = "Version/milestone the summary was scoped to, null when unscoped.")
        Integer versionId,
        @Schema(description = "Open / closed / total issue counts for the scope.", requiredMode = Schema.RequiredMode.REQUIRED)
        ...
        @Schema(description = "Open-issue counts grouped by status name.", requiredMode = Schema.RequiredMode.REQUIRED)
        Map<String, Integer> byStatus,
        @Schema(description = "Open-issue counts grouped by tracker name.", requiredMode = Schema.RequiredMode.REQUIRED)
        Map<String, Integer> byTracker,
        @Schema(description = "Open-issue counts grouped by priority name.", requiredMode = Schema.RequiredMode.REQUIRED)
        Map<String, Integer> byPriority,
        @Schema(description = "Per-assignee workload — total and overdue counts.", requiredMode = Schema.RequiredMode.REQUIRED)
        ...
        int overdue,
        @Schema(description = "Estimated vs spent hours for the analyzed open set.", requiredMode = Schema.RequiredMode.REQUIRED)
        HoursSummary hours
) {

    @Schema(description = "Per-assignee workload entry.")
    public record Assignee(
            @Schema(description = "Display name of the assignee, or 'Unassigned' bucket.", requiredMode = Schema.RequiredMode.REQUIRED)
            String assignee,
