...
public record UserWorkload(
        ...
        int overdue,
        @Schema(description = "Per-project breakdown of the user's workload.", requiredMode = Schema.RequiredMode.REQUIRED)
        List<ProjectShare> byProject,
        @Schema(description = "Up to 10 most important open issues for the user, sorted by priority then by due date.", requiredMode = Schema.RequiredMode.REQUIRED)
        List<IssueSummary> topIssues
) {

    @Schema(description = "Workload contribution from a single project.")
    public record ProjectShare(
            @Schema(description = "Project display name.", requiredMode = Schema.RequiredMode.REQUIRED)
            String project,
            ...
            @Schema(description = "Open-issue counts grouped by priority name within this project.", requiredMode = Schema.RequiredMode.REQUIRED)
