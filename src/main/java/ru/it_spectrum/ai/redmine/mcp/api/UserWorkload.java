...
public record UserWorkload(
        ...
        int userId,
        @Schema(description = "Display name of the user. May be a placeholder like 'User #42' when only an id was provided.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
        String userName,
        ...
        boolean truncated,
        @Schema(description = "Estimated vs spent hours across the analyzed set.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
        HoursSummary hours,
        @Schema(description = "Number of analyzed issues whose due date is in the past.", requiredMode = Schema.RequiredMode.REQUIRED)
        int overdue,
        @Schema(description = "Per-project breakdown of the user's workload.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
        List<ProjectShare> byProject,
        @Schema(description = "Up to 10 most important open issues for the user, sorted by priority then by due date.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
        List<IssueSummary> topIssues
) {

    @Schema(description = "Workload contribution from a single project.")
    public record ProjectShare(
            @Schema(description = "Project display name.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
            String project,
            ...
            @Schema(description = "Open-issue counts grouped by priority name within this project.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
