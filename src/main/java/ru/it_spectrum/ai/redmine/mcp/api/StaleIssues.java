...
public record StaleIssues(
        @Schema(description = "Project identifier the search was scoped to.", requiredMode = Schema.RequiredMode.REQUIRED)
        String projectId,
        ...
        int limit,
        @Schema(description = "Stale issues that crossed the threshold.", requiredMode = Schema.RequiredMode.REQUIRED)
        List<Entry> issues,
    ...
    public record Entry(
            @Schema(description = "The stale issue.", requiredMode = Schema.RequiredMode.REQUIRED)
            IssueSummary issue,
