...
public record BlockerChain(
        @Schema(description = "The issue the chain is centred on.", requiredMode = Schema.RequiredMode.REQUIRED)
        Issue root,
        @Schema(description = "Issues that block the root (recursively), each annotated with its depth from the root.", requiredMode = Schema.RequiredMode.REQUIRED)
        List<Node> blockedBy,
        @Schema(description = "Issues that depend on the root (recursively), each annotated with its depth from the root.", requiredMode = Schema.RequiredMode.REQUIRED)
        List<Node> blocks,
    ...
    public record Node(
            @Schema(description = "Issue at this position in the chain.", requiredMode = Schema.RequiredMode.REQUIRED)
            Issue issue,
