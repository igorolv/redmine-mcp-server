...
public record BlockerChain(
        @Schema(description = "The issue the chain is centred on.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
        Issue root,
        @Schema(description = "Issues that block the root (recursively), each annotated with its depth from the root.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
        List<Node> blockedBy,
        @Schema(description = "Issues that depend on the root (recursively), each annotated with its depth from the root.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
        List<Node> blocks,
    ...
    public record Node(
            @Schema(description = "Issue at this position in the chain.", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
            Issue issue,
