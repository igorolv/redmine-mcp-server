...
Извлечённый текст вложений кэшируется, чтобы повторные запросы не скачивали и не разбирали один и тот же файл заново.

- Небольшой извлечённый текст хранится в памяти процесса.
- Текст больше 1 MB символов сохраняется во временный файл ОС через `Files.createTempFile`.
- TTL кэша — 5 минут.
- При штатном завершении процесса временные файлы удаляются.
- При аварийном завершении процесса временные файлы могут остаться в системной временной директории. Их имена начинаются с `redmine-att-`.
