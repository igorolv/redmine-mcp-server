...
| Tool | Описание |
|---|---|
| `getIssueFullContext` | Полный контекст задачи одним вызовом: описание, единый список окружающих задач с ролями (`parent`, `sibling`, `child`, `related`), вложения с извлечением текста (PDF/DOCX), последние комментарии и флаги усечения. Заменяет 10+ отдельных вызовов. Параметры: `issueId` |
