...
read-only; опционально можно включить ограниченный набор операций записи для задач.
...
`REDMINE_MCP_WRITE_ENABLED=true` к ним добавляются ещё 4 инструмента записи.
...
AI-изменений описание создаваемой задачи, изменяемое описание и новые примечания получают префикс
`AI_EDIT:`, а имена загруженных файлов — `AI_EDIT__`.

Редактирование и удаление существующих примечаний не реализовано: используемый Redmine 4.0.4
не предоставляет для этого совместимый REST API. Запись wiki и трудозатрат также не входит в
...
| `REDMINE_MCP_WRITE_ENABLED` | Добавляет в `tools/list` инструменты `createIssue`, `updateIssue`, `addIssueNote`, `attachFileToIssue`; по умолчанию `false` |
...
- По умолчанию все MCP-инструменты read-only. При `REDMINE_MCP_WRITE_ENABLED=true` становятся доступны четыре явно перечисленных инструмента записи задач; сервер не расширяет права API-пользователя и не обходит workflow Redmine.
...
│   │   ├── Project.java
│   │   ├── IssueMutationResult.java        — стабильный результат операций записи
│   │   └── ...                             — DTO ответов инструментов и аналитики
│   ├── client/
│   │   ├── RedmineClient.java              — read-only обёртка над Redmine REST API
│   │   ├── RedmineMutationClient.java      — опциональные POST/PUT для задач
