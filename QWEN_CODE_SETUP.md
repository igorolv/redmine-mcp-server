...
Эта инструкция описывает только установку и подключение **Redmine MCP Server**. Предполагается, что Qwen Code уже установлен и настроен для работы с выбранной моделью.

## Что понадобится

- Установленный и настроенный Qwen Code.
- Доступ к корпоративному Redmine из той среды, где запускается Qwen Code.
...
```json
{
  "mcpServers": {
    "redmine": {
      ...
      "args": [
        "-jar",
        "C:/absolute/path/to/redmine-mcp-server/build/libs/redmine-mcp-server.jar"
      ],
      "env": {
...
Если `REDMINE_URL` и `REDMINE_API_KEY` уже заданы как системные переменные окружения, секцию `env` можно не указывать.
...
2. Убедитесь, что при старте Qwen Code сервер `redmine` подключился без ошибок.
...
Сервер предоставляет **40 read-only MCP tools**. Он не изменяет данные в Redmine.
...
| Вложения | `listAttachments`, `getAttachmentContent`, `getImageAttachment`, `searchAttachmentContent` |
